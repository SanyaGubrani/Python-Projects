...
# Function to save password data
def save():
    ...
    if len(website) == 0 or len(password) == 0:
        messagebox.showinfo(title="Oops", message="Please enter both website and password.")
    else:
        try:
...
# Function to find a password for a given website
def find_password():
    website = website_entry.get()
    try:
...
window.config(bg="#defffe")  

# Logo
canvas = Canvas(window, height=200, width=200, bg="#defffe", highlightthickness=0)
logo_img = PhotoImage(file="icon.png")
canvas.create_image(100, 100, image=logo_img)
canvas.grid(row=0, column=1, columnspan=2)

# Labels
website_label = Label(window, text="Website:", bg="#defffe")
website_label.grid(row=1, column=0, padx=5, pady=5, sticky="e")
email_label = Label(window, text="Email/Username:", bg="#defffe")
email_label.grid(row=2, column=0, padx=5, pady=5, sticky="e")
password_label = Label(window, text="Password:", bg="#defffe")
password_label.grid(row=3, column=0, padx=5, pady=5, sticky="e")

# Entries
website_entry = Entry(window, width=35)
website_entry.grid(row=1, column=1, columnspan=2, padx=5, pady=5, sticky="w")
website_entry.focus()
email_entry = Entry(window, width=35)
email_entry.grid(row=2, column=1, columnspan=2, padx=5, pady=5, sticky="w")
email_entry.insert(0, "sanya@gmail.com")
